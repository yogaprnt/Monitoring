...
class AkademikController extends Controller
{
    ...
    {
        $user     = auth()->user();
        $akademik = Akademik::where('input_by', $user->id)->latest()->get();
        return view('staff.createakademik', compact('akademik', 'user'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            if ($request->hasFile('file_pendukung')) {
                $filePendukung = $request->file('file_pendukung')
                    ->store('file-pendukung-akademik', 'public');
            }
            ...
            report($e);
            return back()->withInput()->with('error', 'Data akademik gagal disimpan.');
        }
        ...
        return redirect()->route('akademik.index')
            ->with('success', 'Data akademik berhasil disubmit dan menunggu review Asisten Manager.');
    }

    public function edit($id)
    {
        $user     = auth()->user();
        $item     = Akademik::where('input_by', $user->id)->findOrFail($id);
        ...
        $akademik = Akademik::where('input_by', $user->id)->latest()->get();
        return view('staff.createakademik', compact('akademik', 'user', 'item'));
    }

    public function update(Request $request, $id)
    {
        $item = Akademik::where('input_by', auth()->id())->findOrFail($id);
        ...
        try {
            ...
            report($e);
            return back()->withInput()->with('error', 'Data akademik gagal diperbarui.');
        }
        ...
        return redirect()->route('akademik.index')
            ->with('success', 'Data akademik berhasil diperbaiki dan dikirim kembali.');
    }
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            report($e);
            return redirect()->route('akademik.index')->with('error', 'Data gagal dihapus.');
        }
