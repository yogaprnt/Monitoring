...
@section('title', 'Approval Asisten Manager - RI CCSL')
...
<div class="content">
    ...
    <div class="approval-banner">
        <div class="banner-text">
            <div class="banner-title">Review Data <span style="opacity:.75">Asisten Manager</span></div>
            <div class="banner-sub">Tinjau dan setujui data yang disubmit Staff sebelum diteruskan ke Manager</div>
        </div>
    ...
    <!-- ALERT -->
    @if($jumlahMenunggu > 0)
    <div class="alert-banner" style="background:#eff6ff;border:1px solid #bfdbfe;color:#1e40af;">
        <i class="fas fa-hourglass-half" style="color:#3b82f6;"></i>
        <span>Ada <strong>{{ $jumlahMenunggu }} data</strong> yang telah disubmit Staff dan menunggu review Anda.</span>
    </div>
    @else
    <div class="alert-banner" style="background:#f0fdf4;border:1px solid #bbf7d0;color:#15803d;">
        <i class="fas fa-check-circle" style="color:#22c55e;"></i>
        <span>Semua data sudah direview. Tidak ada yang menunggu persetujuan Anda saat ini.</span>
    </div>
    @endif
    ...
    <div class="table-card">
        <div class="table-header">
            <div>
                <div class="table-title">Data Menunggu Review</div>
                <div class="table-sub">Semua data berstatus <em>submitted</em> dari Staff akan ditampilkan di sini</div>
            </div>
            <span class="count-pill">{{ $approvals->count() }} Data</span>
        </div>
        <div style="overflow-x:auto;">
            <table class="tbl">
                ...
                <tbody>
                    ...
                    $tc = $tipeMap[$item->tipe] ?? ['bg'=>'#f1f5f9','color'=>'#475569','icon'=>'fa-file'];
                    @endphp
                    <tr>
                        ...
                        <td class="judul-cell">
                            <div class="judul-main" title="{{ $item->judul }}">{{ $item->judul }}</div>
                        </td>
                        <td>
                            <div class="penginput-cell">
                                <span class="penginput-avatar">{{ strtoupper(substr($item->penginput?->name ?? '?', 0, 1)) }}</span>
                                <span class="penginput-name">{{ $item->penginput?->name ?? '-' }}</span>
                            </div>
                        </td>
                        <td class="date-cell">{{ optional($item->created_at)->format('d M Y H:i') ?? '-' }}</td>
                        <td>
                            <span class="status-badge" style="background:#fef9c3;color:#a16207;">
                                <i class="fas fa-clock" style="font-size:10px;"></i>
                                Submitted
                            </span>
                        </td>
                        <td style="text-align:center;">
                            <a href="{{ route('asisten_manager.item.detail', ['tipe' => strtolower($item->tipe), 'id' => $item->id]) }}"
                               class="btn-detail">
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <div class="empty-title">Tidak ada data menunggu approval</div>
                                <div class="empty-sub">Data staff berstatus submitted akan muncul di sini.</div>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <form id="rejectForm" method="POST">
            ...
            <div class="modal-body">
                ...
                <textarea id="catatan_reject" name="catatan_reject" rows="4" maxlength="1000" required
                    placeholder="Jelaskan data yang harus diperbaiki oleh staff..."
...
function openRejectModal(button) {
    const url   = button.dataset.rejectUrl;
