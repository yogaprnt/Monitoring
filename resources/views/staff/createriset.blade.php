...
@section('title', 'Kinerja Riset - RI CCSL')
...
@php
    $judulOptions = [
        "Artikel/paper yang diterbitkan di jurnal internasional bereputasi (terindeks Scopus/WoS)",
        "Artikel/paper yang diterbitkan di jurnal nasional terakreditasi (Sinta 1-2)",
        "Buku/book chapter yang diterbitkan",
        "Hak Kekayaan Intelektual (Paten, Hak Cipta, dll.)",
        "Penelitian dengan sumber dana internasional",
        "Penelitian dengan sumber dana nasional (DIKTI, BRIN, dll.)",
        "Penelitian dengan sumber dana industri/mitra",
        "Kerjasama riset internasional (MoU/MoA aktif)",
        "Prototype/produk hasil riset yang dikembangkan",
    ];
    $coeOptions = [
...
<div class="content">
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Kinerja Riset</h1>
        <p class="text-gray-500 text-sm mt-1">Kelola data kinerja riset yang sedang berjalan</p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 px-6 py-4 mb-4">
        <div class="flex items-center justify-between mb-4">
            <span class="font-semibold text-gray-700">{{ $isEdit ? 'Edit Riset' : 'Tambah Kinerja Riset Baru' }}</span>
            @if(!$isEdit)
        ...
        <div id="formTambah" class="{{ $isEdit || $errors->any() ? '' : 'hidden' }}">
            <form action="{{ $isEdit ? route('riset.update', $item->id) : route('riset.store') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="triwulan" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#1a2e4a]">
                    ...
                    <div>
                        ...
                        <select name="tahun" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#1a2e4a]">
                    ...
                    <div class="col-span-2">
                        ...
                        <select name="judul" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#1a2e4a]">
                            <option value="">-- Pilih Judul --</option>
                            @foreach($judulOptions as $judul)
                                <option value="{{ $judul }}" {{ old('judul', $item->judul ?? '') == $judul ? 'selected' : '' }}>{{ $judul }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">COE</label>
                        <select name="coe" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#1a2e4a]">
                            <option value="">-- Pilih COE --</option>
                            @foreach($coeOptions as $coe)
                                <option value="{{ $coe }}" {{ old('coe', $item->coe ?? '') == $coe ? 'selected' : '' }}>{{ $coe }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                ...
                <div class="flex items-center space-x-3 pt-1">
                    <button type="submit" class="px-5 py-2 text-sm text-white bg-green-600 hover:bg-green-700 rounded-lg flex items-center space-x-2">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100">
            <h2 class="font-semibold text-gray-700">Data Kinerja Riset</h2>
        </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-[#1a2e4a] text-white text-xs uppercase tracking-wider">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left font-medium">Judul Riset</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4">
                            @if($data->file_pendukung)
                                <a href="{{ asset('storage/'.$data->file_pendukung) }}" target="_blank" class="text-blue-600 hover:underline">Lihat</a>
                            @else
                                -
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4">
                            @if($data->canBeModifiedByStaff())
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('riset.edit', $data->id) }}"
                    ...
                    <tr>
                        <td colspan="8" class="px-6 py-10 text-center text-gray-400">
                            <i class="fas fa-flask text-3xl mb-2 block text-gray-300"></i>
